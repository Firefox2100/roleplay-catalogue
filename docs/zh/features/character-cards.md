...
角色扮演资源库使用 SillyTavern 角色卡格式作为核心内容类型。无论是从头创建新角色，还是上传已有角色卡，资源库都与 SillyTavern 广泛使用的 V2 和 V3 格式保持兼容。资源库中的角色卡用于存储草稿数据、维护完整的版本历史，并可关联最多 50 本来自资源库的世界书。

## 使用方法

### 创建或上传角色卡

以 **JSON 文件**（V2 或 V3）或嵌入 SillyTavern 卡牌数据的 **PNG 文件**形式上传角色卡。资源库会自动检测并解析你上传的格式。

发布时，资源库会生成一个完整的、自包含的角色卡：所有关联的世界书会直接合并到输出 JSON 中，该文件无需任何依赖即可直接用于 SillyTavern。
...
如果上传的是 PNG 格式角色卡，资源库会从 `tEXt` 块中读取嵌入的 JSON，并同时**自动设置角色卡的封面图**。
...
一张角色卡最多可以关联 **50 本 世界书**。关联方式包括：

- **世界书的草稿**（实时 — 下一次发布前，卡牌会跟随 世界书的所有编辑更改）。
...
- **合并的世界书 = 可携带卡牌。** 发布时，关联的世界书会被编译到输出 JSON 中。这意味着无论关联的世界书是否仍在资源库中存在，你的角色卡始终包含完整的定义。
